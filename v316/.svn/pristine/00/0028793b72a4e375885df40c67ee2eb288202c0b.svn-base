...
void wTalk::doEvent(mEvent *e)
{
	...
	if (e->type == mEvent::KeyPress) {
		if (m_menu_select == 1) {
			...
			} else if (e->wParam == KEY_M_F5) {
				m_menu_select = 1;
				m_led_select = LED_RTS;
				gettimeofday(&led_tv, NULL);
				loadNavigation(m_menu_select);
			}
		} else {
			if (e->wParam == KEY_M_F2) {
...
void wTalk::loadNavigation(int select)
{
	char buf[128];
