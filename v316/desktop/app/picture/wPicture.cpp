...
wPicture::wPicture(): mWindow("picture")
{
	...
	m_position = pictureUtil.getCurrentPosition();
	m_picureNum = pictureUtil.getPictureNum();

	m_unReadPicureNum = 0;
	std::string name("");
	for (int i = 0; i < m_picureNum; i++) {
...
void wPicture::loadPicure(int position)
{
	...
	if (!name.empty()
		...
		loadInfos(name);
		if (pictureUtil.isUnReadPicture(name)) {
			snprintf(buf, sizeof(buf), "(%d/%d)", position+1, m_unReadPicureNum);
...
void wPicture::modeChange(int mode)
{
	...
	if (mode == 2) {
		...
		if (m_picureNum == 0) 
			m_number.loadUrl(m_style, "number2");
		else if (m_picureNum > 9)
			m_number.loadUrl(m_style, "number4");
		else
			m_number.loadUrl(m_style, "number3");
	} else {
		m_number.hide();
	}

	snprintf(buf, sizeof(buf), "%d", m_picureNum);
